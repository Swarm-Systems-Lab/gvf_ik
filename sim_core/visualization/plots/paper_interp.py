...
class PlotInterp:
    ...
    def config_axes(self, y, x_step=1, y_step=10):
        ...
        """
        self.ax.set_xlabel(r"$\dot{\bar x}$ [m/s]")
        self.ax.set_ylabel(r"$A$ [m]")
    ...
    def plot(self):
        ...
        for i in range(n_omega):
            ...
            if omega_list[i] > np.pi:
                ...
                self.ax.text(x_dot_mean[(i+1)*self.N - 1]+0.5, A[(i+1)*self.N - 1]+1, label)

        
        # Add legend
        self.ax.plot([], [], "k", label=r"$k_A \sqrt{v^2 - \dot{\bar x}^2}/w$")
        self.ax.legend()
        # -----------------------------------------------------------------------------
